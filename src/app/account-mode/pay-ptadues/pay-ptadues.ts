...
@Component({
  ...
  imports: [ToastModule,
    ...
    DialogModule,
    MessageModule],
  templateUrl: './pay-ptadues.html',
  styleUrl: './pay-ptadues.css',
    providers: [MessageService]
})
export class PayPTADues {
  ...
  getAccountHistory = () => {
    ...
    this.learnerAccountservice.loadptadueHistory(data).subscribe((response: any) => {
      if (response?.message) {
  ...
  loadFeebalance = () => {
    ...
    this.learnerAccountservice.loadptaduebalance(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
        this.messageservice.add({ severity: 'error', summary: 'Messgae', detail: this.message });
      } else {
  ...
  submiPayment = () => {
    ...
    this.learnerAccountservice.postptaduePayment(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
  ...
  genranCode = () => {
    ...
    this.PaymentNumber = "LPTA-LDE-" + new Date().getFullYear() + randomInteger
  }
}