...
@Component({
  ...
  templateUrl: './ptadues-billing.html',
  styleUrl: './ptadues-billing.css',
})
export class PtaduesBilling {
...
billHistory:Canteen[]=[]
  ...
  ngOnInit(): void {
    ...
    this.genranCode()
    this.getPtaBillhistory()
  }

  getPtaCurrentBills() {
return this.billservice.getPtaCurrentBills().subscribe((response: any) => {
      if (response?.data) {
...
dropPtaBill(_t163: any) {
let data={
  billID:_t163.ptabillnumber
}
console.log(data)
this.billservice.dropPtaBill(data).subscribe((response:any)=>{
 if(response?.message){
    this.message=response?.message
  }else{
    if(response?.success){
...
this.billservice.submitPtaBiil(data).subscribe((response:any)=>{
  if(response?.message){
    this.message=response?.message
  }else{
    if(response?.success){
...
getPtaBillhistory=()=>{
 return this.billservice.getPtaBillhistory().subscribe((response: any) => {
      if (response?.data) {
        this.billHistory = response?.data
      this.loading.set(false)
      } else {
