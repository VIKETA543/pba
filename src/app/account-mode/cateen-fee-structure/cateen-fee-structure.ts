...
@Component({
  ...
  imports: [PanelModule,
    ...
    ButtonModule,
    ToggleSwitchModule,],
  templateUrl: './cateen-fee-structure.html',
  styleUrl: './cateen-fee-structure.css',
})
export class CateenFeeStructure {
  ...
  ngOnInit(): void {
    ...
    this.loadCurrentCanteenbill_schedule()
    this.getFeeinArrears()
    this.setAmountDue();
    this.genranCode()
    this.loadcanteenfeehistory_schedule()
        this.termdetails();
  ...
  genranCode = () => {
    ...
    this.billID = "CNT-BL-" + new Date().getFullYear() + randomInteger
  }
  ...
  loadCurrentCanteenbill_schedule = () => {
    let data = {
      grade: this.CurrentGrade,
      term: this.currentTerm
    }
    console.log('current bill',data)
    this.billService.loadCurrentCanteenbill_schedule(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
        this.messageService.add({
          severity: 'success', summary: 'Messgae', detail: this.message, key: 'br',
  ...
  getFeeinArrears = () => {
    ...
    this.billService.Loadcateen_fee_arears(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
        this.messageService.add({
  ...
  submitCanteenFee = () => {
    ...
    this.billService.submitcurrentCanteenfee_schedule(data).subscribe((response: any) => {
  ...
  loadcanteenfeehistory_schedule = () => {
    let data = {
      AdmissionNumber: this.AdmissionNumber,
    }
    this.billService.loadcanteenfeehistory_schedule(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
        this.messageService.add({
          severity: 'danger', summary: 'Messgae', detail: this.message,life: 3000
  ...
  dropEntry(_t111: any) {
    ...
    this.billService.dropcanteefee_schedule(data).subscribe((response: any) => {
      if (response?.message) {
        this.message = response?.message
        this.messageService.add({
